...
static auto gammaShader = std::make_unique<npb::GammaShader<npb::Rgb8Color>>(2.2f, true, false);

static auto leds = npb::factory::makeWs2812xOwningShaderBusDriverPixelBus<npb::RpPioOneWireTransport>(
    PixelCount,
    npb::ChannelOrder::GRB,
    std::move(gammaShader),
    transportConfig);
...
void setup()
{
    ...
    Serial.println("RP2040 WS2812x BusDriver + shader example");
    leds.begin();
}
