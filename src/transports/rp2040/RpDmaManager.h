  ...
  public:
    class ChannelLease
    {
      public:
        ...
        void startTransfer(span<const uint8_t> data, volatile void* writeAddress, uint dreq, bool byteSwap = true,
                           bool readIncrement = true, bool writeIncrement = false)
        {
            ...
            const dma_channel_transfer_size transferSize =
                (data.size() % 4 == 0) ? DMA_SIZE_32 : ((data.size() % 2 == 0) ? DMA_SIZE_16 : DMA_SIZE_8);
            ...
            _owner->startTransferOnChannel(static_cast<uint>(_channel), data.data(), writeAddress, transferCount,
                                           transferSize, dreq, byteSwap, readIncrement, writeIncrement);
        }
    ...
    static uint32_t computeFifoCacheEmptyDeltaUs(uint32_t bitPeriodNs)
    {
        ...
        const uint64_t fifoDrainNs = static_cast<uint64_t>(bitPeriodNs) * static_cast<uint64_t>(8U) *
    ...
    void startTransferOnChannel(uint dmaChannel, const void* readAddress, volatile void* writeAddress,
                                uint transferCount, dma_channel_transfer_size transferSize, uint dreq, bool byteSwap,
                                bool readIncrement, bool writeIncrement)
    {
        if (!hasChannel() || channel() != dmaChannel || readAddress == nullptr || writeAddress == nullptr ||
            transferCount == 0)
        {
            return;
        }
