...
{
  public:
    template <typename TColor> static TColor parseHex(const char* str, const char* colorOrder = nullptr)
    {
        TColor result{};
        if (str == nullptr)
        {
    ...
    static void fillHex(const TColor& color, span<uint8_t> resultBuffer, const char* colorOrder = nullptr,
        ...
        for (size_t logicalChannel = 0; logicalChannel < ChannelCount; ++logicalChannel)
        {
            ...
            for (size_t digit = 0; digit < DigitsPerComponent; ++digit)
            {
                ...
                const uint8_t nibble =
                    static_cast<uint8_t>((value >> nibbleShift) & static_cast<typename TColor::ComponentType>(0x0F));
  ...
  private:
    template <typename TColor> static constexpr const char* defaultColorOrder()
    {
        constexpr size_t ChannelCount = static_cast<size_t>(TColor::ChannelCount);
