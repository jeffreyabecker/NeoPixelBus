...
template <typename TColor, typename = std::enable_if_t<ColorComponentTypeIs<TColor, uint8_t>>>
...
class GammaShader : public IShader<TColor>
{
  public:
    ...
    explicit GammaShader(SettingsType settings = {})
        : gammaCorrectCol{settings.enableColorGamma}, gammaCorrectBri{settings.enableBrightnessGamma},
    ...
    uint8_t gamma8_cal(uint8_t b, float gamma) const
    {
        ...
        return static_cast<uint8_t>(static_cast<int>(powf(static_cast<float>(b) / 255.0f, gamma) * 255.0f + 0.5f));
    }
    ...
    uint8_t gamma8_slow(uint8_t b) const { return gammaT[b]; }

    uint8_t gamma8(uint8_t value) const { return gammaT[value]; }
  ...
  private:
    ...
    static constexpr uint8_t rFromColor(uint32_t color) { return static_cast<uint8_t>(color >> 16); }

    static constexpr uint8_t gFromColor(uint32_t color) { return static_cast<uint8_t>(color >> 8); }

    static constexpr uint8_t bFromColor(uint32_t color) { return static_cast<uint8_t>(color); }

    static constexpr uint8_t wFromColor(uint32_t color) { return static_cast<uint8_t>(color >> 24); }
