...
template <typename TColor> struct CurrentLimiterShaderSettings
{
...
template <typename TColor> class CurrentLimiterShader : public IShader<TColor>
{
  public:
    ...
    explicit CurrentLimiterShader(SettingsType settings)
        : _maxMilliamps{settings.maxMilliamps}, _controllerMilliamps{settings.controllerMilliamps},
          _standbyMilliampsPerPixel{settings.standbyMilliampsPerPixel}, _rgbwDerating{settings.rgbwDerating},
    ...
    void apply(span<TColor> colors) override
    {
        ...
        uint64_t estimatedMilliamps = (weightedDraw / maxComponent) + _controllerMilliamps +
        ...
        {
            scaleAll(colors, 0);
            _lastEstimatedMilliamps = _controllerMilliamps + static_cast<uint32_t>(_standbyMilliampsPerPixel) *
...
template <typename TColor> using CurrentLimiterShaderSettings = shaders::CurrentLimiterShaderSettings<TColor>;

template <typename TColor> using CurrentLimiterShader = shaders::CurrentLimiterShader<TColor>;
