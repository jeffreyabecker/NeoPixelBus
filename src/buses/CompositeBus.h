...
namespace lw::busses
{
    ...
    {
    public:
        static_assert(sizeof...(TBuses) > 0,
                      "CompositeBus requires at least one bus type.");
        ...
        explicit CompositeBus(TBuses... buses)
            ...
            , _busPointers(makeBusPointers(_buses))
            , _pixelChunks(collectChunks(_busPointers))
        ...
        BusesTupleType &buses()
        {
            return _buses;
        }

        const BusesTupleType &buses() const
        {
            return _buses;
        }
    ...
    private:
        ...
        static std::array<BusBaseType *, sizeof...(TBuses)> makeBusPointers(BusesTupleType &buses)
        {
            return makeBusPointers(buses,
                                   std::index_sequence_for<TBuses...>{});
        }
