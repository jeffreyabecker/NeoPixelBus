...
struct Ws2812xProtocolSettings : public ProtocolSettings
{
    ...
    struct TransportHasInvert<TTransportSettings, std::void_t<decltype(std::declval<TTransportSettings&>().invert)>>
        : std::true_type
    {
    };

    template <typename TTransportSettings>
    static void applyTransportDefaults(const Ws2812xProtocolSettings& settings, TTransportSettings& transportSettings)
    {
...
class Ws2812xProtocol : public IProtocol<TInterfaceColor>
{
  public:
    ...
    static constexpr size_t requiredBufferSize(PixelCount pixelCount, const SettingsType& settings)
    {
        ...
        const size_t prefixResetBytes =
            transports::OneWireEncoding::computeResetBytes(settings.timing, 0, settings.prefixResetMultiplier);
        const size_t suffixResetBytes =
            transports::OneWireEncoding::computeResetBytes(settings.timing, 0, settings.suffixResetMultiplier);
        return prefixResetBytes + payloadBytes + suffixResetBytes;
    }

    template <typename TTransportSettings>
    static void normalizeTransportSettings(uint16_t, const SettingsType& settings,
    ...
    Ws2812xProtocol(PixelCount pixelCount, SettingsType settings)
        : IProtocol<InterfaceColorType>(pixelCount), _settings{std::move(settings)},
          _channelOrder{resolveChannelOrder(_settings.channelOrder)}, _channelCount{resolveChannelCount(_channelOrder)},
          _rawSizeData{bytesNeeded(pixelCount, _channelCount)}, _sizeData{requiredBufferSize(pixelCount, _settings)}
    {
    }

    Ws2812xProtocol(PixelCount pixelCount, const char* channelOrder)
        : Ws2812xProtocol{pixelCount, Ws2812xProtocolSettings{{}, channelOrder, transports::timing::Ws2812x}}
    {
    }
    ...
    Ws2812xProtocol(Ws2812xProtocol&& other) noexcept
        : IProtocol<InterfaceColorType>(other._pixelCount), _settings{std::move(other._settings)},
          _channelOrder{other._channelOrder}, _channelCount{other._channelCount}, _rawSizeData{other._rawSizeData},
    ...
    void update(span<const InterfaceColorType> colors, span<uint8_t> buffer = span<uint8_t>{}) override
    {
        ...
        const size_t encodedSize = transports::OneWireEncoding::encodeWithResets(
            _frameData.data(), _rawSizeData, _frameData.data(), _frameData.size(), _settings.timing, 0,
            _settings.prefixResetMultiplier, _settings.suffixResetMultiplier, ProtocolIdleHigh);
        if (encodedSize == 0)
        {
            return;
        }
  ...
  private:
    ...
    static constexpr void appendWireComponent(span<uint8_t> pixels, size_t& offset,
