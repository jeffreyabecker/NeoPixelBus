                                                   ...
                                                   DebugProtocolSettingsT<TWrappedProtocol, TWritable>>
{
  public:
    ...
    using BaseType = ProtocolDecoratorBase<DebugProtocol<TWrappedProtocol, TWritable>, TWrappedProtocol, ColorType,
                                           DebugProtocolSettingsT<TWrappedProtocol, TWritable>>;
    ...
    DebugProtocol(PixelCount pixelCount, SettingsType settings)
        : BaseType(pixelCount, TWrappedProtocol{pixelCount, settings.wrapped}, std::move(settings))
    {
    }
    ...
    void afterBegin()
    {
        ...
        if (settings.output != nullptr)
        {
            ...
            const size_t countLength =
                formatUnsignedDecimal(countBuffer, sizeof(countBuffer), static_cast<unsigned long>(this->pixelCount()));
            if (countLength > 0)
            {
    ...
    void beforeUpdate(span<const ColorType> colors, span<uint8_t> buffer = span<uint8_t>{})
    {
        ...
        if (settings.output != nullptr)
        {
            ...
            const size_t countLength =
                formatUnsignedDecimal(countBuffer, sizeof(countBuffer), static_cast<unsigned long>(colors.size()));
            if (countLength > 0)
            {
                writeBytes(reinterpret_cast<const uint8_t*>(countBuffer), countLength);
            }
