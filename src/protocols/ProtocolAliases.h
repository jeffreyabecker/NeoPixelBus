...
template <typename TWrappedSpec, typename TWrappedSettings, typename = void>
struct WrappedSpecHasNormalizeSettings : std::false_type
{
};

template <typename TWrappedSpec, typename TWrappedSettings>
struct WrappedSpecHasNormalizeSettings<
    TWrappedSpec, TWrappedSettings,
...
template <typename TInterfaceColor = lw::Rgb8Color, typename TDefaultChannelOrder = lw::ChannelOrder::BGR,
...
struct DotStar
{
    ...
    using EffectiveInterfaceColor =
        std::conditional_t<lw::ColorAtLeastAsLarge<TInterfaceColor, TStripColor>, TInterfaceColor, TStripColor>;
...
template <typename TInterfaceColor = lw::Rgb16Color, typename TDefaultChannelOrder = lw::ChannelOrder::BGR,
...
template <typename TColor = lw::Rgb8Color> struct None
{
...
template <typename TWrappedProtocolSpec = None<lw::Rgb8Color>> struct Debug
{
...
template <typename TInterfaceColor = lw::Rgbw8Color> struct Tm1814
{
    ...
    static SettingsType normalizeSettings(SettingsType settings)
    {
        return SettingsType::template normalizeForColor<ColorType>(std::move(settings), "WRGB");
    }
};

template <typename TInterfaceColor = lw::Rgb8Color> struct Tm1914
{
    using ColorType = TInterfaceColor;
    ...
    static SettingsType normalizeSettings(SettingsType settings)
    {
        return SettingsType::template normalizeForColor<ColorType>(std::move(settings), lw::ChannelOrder::GRB::value);
    }
};

template <typename TInterfaceColor = lw::Color, typename TDefaultChannelOrder = lw::ChannelOrder::GRB,
