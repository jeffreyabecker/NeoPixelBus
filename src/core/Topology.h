...
namespace lw
{
    ...
    public:
        ...
        constexpr GridMapping()
            : _value(0)
        {
        }

        constexpr explicit GridMapping(uint8_t value)
            : _value(value)
        {
        }
        ...
        constexpr uint8_t rotation() const
        {
            return static_cast<uint8_t>(_value & RotationMask);
        }

        constexpr QuarterTurn quarterTurn() const
        {
            return static_cast<QuarterTurn>(_value & RotationMask);
        }

        constexpr bool isAlternating() const
        {
            return (_value & AlternatingBit) != 0;
        }

        constexpr bool isColumnMajor() const
        {
            return (_value & ColumnMajorBit) != 0;
        }

        constexpr AxisOrder axisOrder() const
        {
            return isColumnMajor() ? AxisOrder::ColumnsFirst : AxisOrder::RowsFirst;
        }
        ...
        constexpr Components unpack() const
        {
            return Components{axisOrder(), linePattern(), quarterTurn()};
        }

        static constexpr GridMapping make(AxisOrder axisOrder,
                                          LinePattern linePattern,
                                          QuarterTurn quarterTurn)
        {
            const uint8_t family = static_cast<uint8_t>(
                (axisOrder == AxisOrder::ColumnsFirst ? ColumnMajorBit : 0) |
        ...
        friend constexpr bool operator==(GridMapping a, GridMapping b)
        {
    ...
    public:
        ...
        static constexpr uint16_t mapLayout(GridMapping layout,
                                            ...
                                            uint16_t x, uint16_t y)
        {
            ...
            if (!columnMajor)
            {
                ...
                return (ry & 1)
                           ? static_cast<uint16_t>(rowBase + (rw - 1 - rx))
                           : static_cast<uint16_t>(rowBase + rx);
            }
            ...
            return (rx & 1)
                       ? static_cast<uint16_t>(colBase + (rh - 1 - ry))
                       : static_cast<uint16_t>(colBase + ry);
        }
        ...
        static constexpr GridMapping tilePreferredLayout(GridMapping baseLayout,
                                                         bool isOddTileRow,
                                                         bool isOddTileColumn)
        {
            ...
            if (linePattern == GridMapping::LinePattern::Progressive)
            {
                ...
                return GridMapping::make(axisOrder,
                                         linePattern,
                                         quarterTurn);
            }
        ...
        static constexpr Topology linear(size_t length)
        {
            return Topology{TopologySettings{static_cast<uint16_t>(length),
                                             1,
                                             GridMapping::make(GridMapping::AxisOrder::RowsFirst,
                                                               GridMapping::LinePattern::Progressive,
        ...
        constexpr explicit Topology(TopologySettings config)
            : _config(config)
            , _effectiveLayoutByTileParity(makeEffectiveLayoutCache(config))
        ...
        constexpr size_t map(int16_t x, int16_t y) const
        {
            ...
            const GridMapping effectiveLayout = _effectiveLayoutByTileParity[parityIndex(isOddTileRow,
                                                                                          isOddTileColumn)];
    ...
    private:
        static constexpr size_t parityIndex(bool isOddTileRow,
                                            bool isOddTileColumn)
        {
            return static_cast<size_t>((isOddTileRow ? 2u : 0u) |
                                       (isOddTileColumn ? 1u : 0u));
        }
        ...
        static constexpr std::array<GridMapping, 4> makeEffectiveLayoutCache(const TopologySettings &config)
        {
            ...
            return std::array<GridMapping, 4>{
                tilePreferredLayout(config.layout, false, false),
                tilePreferredLayout(config.layout, false, true),
                tilePreferredLayout(config.layout, true, false),
