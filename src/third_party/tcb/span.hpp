...
template <typename T, std::size_t Extent = dynamic_extent> class span
{
  public:
    ...
    template <std::size_t N, typename = typename std::enable_if<Extent == dynamic_extent || N == Extent>::type>
    ...
    constexpr span(std::array<U, N>& arr) noexcept : _data(arr.data()), _size(N)
    {
    }
    ...
    constexpr span(const std::array<U, N>& arr) noexcept : _data(arr.data()), _size(N)
    {
    }

    template <typename U, std::size_t OtherExtent,
              typename = typename std::enable_if<(Extent == dynamic_extent || OtherExtent == Extent ||
                                                  OtherExtent == dynamic_extent) &&
                                                 std::is_convertible<U (*)[], T (*)[]>::value>::type>
    constexpr span(const span<U, OtherExtent>& other) noexcept : _data(other.data()), _size(other.size())
    {
    }
