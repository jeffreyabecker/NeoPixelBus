...
namespace
{
    ...
    public:
        ...
        explicit StubBus(size_t pixelCount,
                         bool ready = true)
            : _pixelsStorage(pixelCount)
            , _chunks{lw::span<TestColor>{_pixelsStorage.data(), _pixelsStorage.size()}}
            , _pixels(lw::span<lw::span<TestColor>>{_chunks.data(), _chunks.size()})
            , _ready(ready)
        {
        }
        ...
        const lw::PixelView<TestColor> &pixels() const override
        {
            return _pixels;
        }
    ...
    void test_composite_bus_pixels_concatenate_child_views(void)
    {
        ...
        lw::busses::CompositeBus<StubBus, StubBus> composite(
            std::move(leftBus),
            std::move(rightBus));
