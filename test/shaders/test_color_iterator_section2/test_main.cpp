...
{
    std::array<TestColor, 6> buffer{TestColor{1, 0, 0, 0, 0}, TestColor{2, 0, 0, 0, 0}, TestColor{3, 0, 0, 0, 0},
                                    TestColor{4, 0, 0, 0, 0}, TestColor{5, 0, 0, 0, 0}, TestColor{6, 0, 0, 0, 0}};

    TestIterator it([&](uint16_t idx) -> TestColor& { return buffer[idx]; }, 2);
...
void test_2_1_2_arithmetic_and_distance_semantics(void)
{
    ...
    TestIterator begin([&](uint16_t idx) -> TestColor& { return buffer[idx]; }, 0);
    TestIterator end([&](uint16_t idx) -> TestColor& { return buffer[idx]; }, static_cast<uint16_t>(buffer.size()));
...
{
    std::array<TestColor, 4> buffer{TestColor{1, 1, 1, 1, 1}, TestColor{2, 2, 2, 2, 2}, TestColor{3, 3, 3, 3, 3},
                                    TestColor{4, 4, 4, 4, 4}};

    TestIterator it([&](uint16_t idx) -> TestColor& { return buffer[idx]; }, 1);
...
void test_2_1_4_comparison_semantics(void)
{
    ...
    TestIterator a([&](uint16_t idx) -> TestColor& { return buffer[idx]; }, 1);
    TestIterator b([&](uint16_t idx) -> TestColor& { return buffer[idx]; }, 3);
    TestIterator c([&](uint16_t idx) -> TestColor& { return buffer[idx]; }, 1);
...
{
    std::array<TestColor, 4> spanBufferA{TestColor{1, 2, 3, 4, 5}, TestColor{6, 7, 8, 9, 10},
                                         TestColor{11, 12, 13, 14, 15}, TestColor{16, 17, 18, 19, 20}};
...
void test_2_2_4_stl_interop_with_std_copy(void)
{
    ...
    {
        std::array<TestColor, 3> sourceBuffer{TestColor{1, 1, 1, 1, 1}, TestColor{2, 2, 2, 2, 2},
                                              TestColor{3, 3, 3, 3, 3}};
...
void test_2_3_2_p0_iterator_arithmetic_overflow_underflow_guarded(void)
{
    ...
    TestIterator low([&](uint16_t idx) -> TestColor& { return buffer[idx % buffer.size()]; }, 0);

    TestIterator high([&](uint16_t idx) -> TestColor& { return buffer[idx % buffer.size()]; },
...
{
    std::array<TestColor, 2> firstBuffer{TestColor{1, 0, 0, 0, 0}, TestColor{2, 0, 0, 0, 0}};
    std::array<TestColor, 2> secondBuffer{TestColor{9, 0, 0, 0, 0}, TestColor{8, 0, 0, 0, 0}};

    TestIterator first([&](uint16_t idx) -> TestColor& { return firstBuffer[idx]; }, 1);

    TestIterator second([&](uint16_t idx) -> TestColor& { return secondBuffer[idx]; }, 1);
