...
uint32_t estimate_pixel_milliamps(lw::span<const Color> colors,
                                  const Settings::ChannelMilliampsMap& milliampsPerChannel, bool rgbwDerating)
{
...
{
    return {Color{100, 120, 140, 160, 180}, Color{10, 20, 30, 40, 50}};
}
...
void test_3_1_2_under_budget_pass_through(void)
{
    ...
    const uint32_t pixelMilliamps = estimate_pixel_milliamps(lw::span<const Color>{frame.data(), frame.size()},
                                                             settings.milliampsPerChannel, settings.rgbwDerating);
...
{
    std::vector<Color> frame{Color{255, 255, 255, 255, 255}, Color{255, 255, 255, 255, 255}};
    ...
    const uint32_t pixelMilliampsBefore = estimate_pixel_milliamps(lw::span<const Color>{frame.data(), frame.size()},
                                                                   settings.milliampsPerChannel, settings.rgbwDerating);
    const uint32_t standby =
        static_cast<uint32_t>(settings.standbyMilliampsPerPixel) * static_cast<uint32_t>(frame.size());
...
{
    std::vector<Color> frame{Color{90, 80, 70, 60, 50}, Color{9, 8, 7, 6, 5}};
    ...
    const uint32_t expected = settings.controllerMilliamps + static_cast<uint32_t>(settings.standbyMilliampsPerPixel) *
...
{
    std::vector<Color> frame{Color{255, 128, 64, 32, 16}, Color{1, 2, 3, 4, 5}};
    ...
    const uint32_t expected =
        pixelMilliamps + settings.controllerMilliamps +
        static_cast<uint32_t>(settings.standbyMilliampsPerPixel) * static_cast<uint32_t>(frame.size());
...
void test_3_2_2_rgbw_derating_enabled(void)
{
    ...
    const uint32_t expectedPixel =
        estimate_pixel_milliamps(lw::span<const Color>{frame.data(), frame.size()}, settings.milliampsPerChannel, true);
...
void test_3_2_3_rgbw_derating_disabled(void)
{
    ...
    const uint32_t expectedPixel = estimate_pixel_milliamps(lw::span<const Color>{frame.data(), frame.size()},
                                                            settings.milliampsPerChannel, false);
    TEST_ASSERT_EQUAL_UINT32(expectedPixel, shader.lastEstimatedMilliamps());
}

void test_3_2_4_standby_current_budget_interaction(void)
{
    std::vector<Color> frame{Color{255, 255, 255, 0, 0}, Color{255, 255, 255, 0, 0}};
...
int main(int argc, char** argv)
{
    ...
    RUN_TEST(test_3_1_1b_channel_letter_assignment_for_milliamps);
