...
| Closed | DMX512 | `src/virtual/protocols/Dmx512Protocol.h` and `src/virtual/transports/Esp8266I2SSelfClockingTransport.h` now present | Gap closed for virtual DMX512 path; keep validating framing/timing parity against legacy behavior |
...
| Closed | TM1814 / TM1914 | `src/virtual/protocols/Tm1814Protocol.h` and `src/virtual/protocols/Tm1914Protocol.h` now provide protocol-owned in-band settings encoding | Keep validating stream parity in `examples-virtual/in-band-settings/main.cpp` |
| Closed | SM168x (3/4/5-channel variants) | `src/virtual/protocols/Sm168xProtocol.h` now provides 3/4/5-channel gain/settings packing | Keep validating trailer packing parity in `examples-virtual/in-band-settings/main.cpp` |

Quick conclusion: there are currently no fully uncaptured chip/protocol families in the new codebase. MBI6033/UCS8903/UCS8904 remain partial parity areas (including 16-bit color-model alignment work where applicable), while Pixie Stream, DMX512, TM1814/TM1914, and SM168x in-band settings are now closed.
