...
- Transport model is unified on `ITransport` with category tags (`TransportTag`, `OneWireTransportTag`, plus `AnyTransportTag` wildcard) and concept-constrained pairing.
...
### 7.0 Concrete Task Breakdown (ordered)

Use this order to keep churn localized and preserve bisectability.

1. **Compat foundation (single PR)**
   - Add/complete:
     - `src/virtual/compat/config.h`
     - `src/virtual/compat/span.h`
     - `src/virtual/compat/optional.h`
     - `src/virtual/compat/traits.h`
     - `src/virtual/compat/utility.h` (`make_unique`, `clamp`, attribute macros)
   - Wire feature macros: `NPB_CXX_STANDARD`, `NPB_HAS_CONCEPTS`, `NPB_HAS_STD_SPAN`, `NPB_HAS_STD_OPTIONAL`.
   - Acceptance: no behavior change; C++23 build still green.

2. **Public seam conversion (high fan-out, no behavior changes)**
   - Convert interfaces to compat wrappers:
     - `src/virtual/protocols/IProtocol.h`
     - `src/virtual/transports/ITransport.h`
     - `src/virtual/IPixelBus.h`
     - `src/virtual/colors/IShader.h`
     - `src/virtual/ResourceHandle.h`
   - Replace direct concept/requires usage with trait-based checks + `static_assert` diagnostics.
   - Acceptance: public headers compile on gcc 10.1 and current toolchain.

3. **Color and channel-order migration slice**
   - Update:
     - `src/virtual/colors/Color.h`
     - any dependent color utility headers under `src/virtual/colors/*`
   - Replace inline constexpr-heavy constructs with C++11-safe alternatives.
   - Acceptance: color indexing behavior unchanged (`'R'/'G'/'B'/'W'/'C'`).
...
4. **Protocol layer migration (family-by-family)**
   ...
   - Convert spans/concepts/optional references to compat abstractions.
   - Acceptance: protocol smoke examples compile and run unchanged.

5. **Transport layer migration (platform partitions)**
   - First shared headers:
     - `src/virtual/transports/OneWireWrapper.h`
     - `src/virtual/transports/OneWireTiming.h`
     - `src/virtual/transports/PrintTransport.h`
     - `src/virtual/transports/DebugTransport.h`
   - Then platform folders independently:
     - `src/virtual/transports/rp2040/*`
     - `src/virtual/transports/esp32/*`
     - `src/virtual/transports/esp8266/*`
     - `src/virtual/transports/nrf52/*`
   - Acceptance: each platform subset compiles when targeted platform macro is active.

6. **Bus/topology composition migration**
   - Update:
     - `src/virtual/buses/PixelBus.h`
     - `src/virtual/buses/SegmentBus.h`
     - `src/virtual/buses/ConcatBus.h`
     - `src/virtual/buses/MosaicBus.h`
     - `src/virtual/topologies/*`
   - Focus on `Optional` behavior parity in resolve/probe paths.
   - Acceptance: no behavioral diffs in out-of-range and unresolved mapping behavior.

7. **Umbrella + examples + docs finalization**
   - Update exports/usages:
     - `src/VirtualNeoPixelBus.h`
     - `examples-virtual/**`
   - Refresh docs:
     - `docs/consumer-virtual-architecture.md`
     - `docs/notes.md`
   - Acceptance: at least one smoke example per protocol/transport family builds under gcc 10.1 lane.

### 7.0.1 Recommended commit slicing

- Commit A: compat layer only.
- Commit B: interface seams (`ITransport`, `IProtocol`, `IPixelBus`, `IShader`, `ResourceHandle`).
- Commit C: colors.
- Commits D1..Dn: protocols by family.
- Commits E1..En: transports by platform.
- Commit F: buses + topologies.
- Commit G: umbrella/docs/examples cleanups.

### 7.0.2 CI gate policy for rollout

- Keep current lane (C++23) required throughout migration.
- Add gcc 10.1 lane as non-blocking initially, then blocking after phase 4.
- Make both lanes required before merge of phase 7.
